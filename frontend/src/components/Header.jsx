...
import { Search, ShoppingCart, User, Heart, Phone, LogOut, ShieldCheck, Package, MapPin, Settings } from "lucide-react";
                ...
                <div ref={accountMenuRef} className="relative p-1">
                  ...
                  >
                    <div className="flex flex-col py-1.5 bg-white border border-gray-100 shadow-xl rounded-xl">
                      ...
                      {["admin", "staff"].includes(user.role_name?.toLowerCase()) && (
                        <Link to="/admin/operations" onClick={() => setIsAccountMenuOpen(false)} className="flex items-center gap-2 px-4 py-2 text-xs font-bold text-gray-700 hover:bg-amber-50 hover:text-amber-600" role="menuitem">
                          <Settings className="h-3.5 w-3.5" /> Vận hành
                        </Link>
                      )}
